...
const Detail = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `https://api.unsplash.com/photos/${params.id}?per_page=20&client_id=sY2cykZbUUSjRNSBmLroQLDp65cQr6unltFGpa1pvhY`
      )
  ...
  return (
    <>
      {imageData.id ? (
        <Container
          fluid
          style={{ backgroundColor: `${imageData?.color}35` }}
          className="p-5"
        >
          <Row>
            <Col md={6}>
            ...
            <Col md={6}>
              <h3>
                Uploaded By: {imageData?.user?.name} (@{imageData?.user?.id})
              </h3>
